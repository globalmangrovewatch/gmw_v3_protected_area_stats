...
class GenTaskCmds(PBPTGenQProcessToolCmds):
    def gen_command_info(self, **kwargs):
        ...
        for protect_area_lyr in protect_area_lyrs:
            ...
            out_dir = os.path.join(out_protect_dir, "hchm_tile_stats")
            if not os.path.exists(out_dir):
                os.mkdir(out_dir)

            for protect_area_tile in protect_area_tiles:
                hgt_img_file = rsgislib.tools.filetools.find_file_none(
                    kwargs["hgt_img_dir"], f"*{protect_area_tile}*.tif"
                )
                if hgt_img_file is None:
                    raise Exception("No HCHM Image file: {}".format(protect_area_tile))
                ...
                out_file = os.path.join(
                    out_dir, "{}_hchm.json".format(protect_area_tile)
                )
                if not os.path.exists(out_file):
                    c_dict = dict()
                    c_dict["hgt_img"] = hgt_img_file
    ...
    def run_gen_commands(self):
        ...
        self.gen_command_info(
            ...
            hgt_img_dir="/home/pete/Documents/gmw_protected_areas/data/gmw_srtm_agb_hchm/hchm_mng_mjr_2020_tif",
