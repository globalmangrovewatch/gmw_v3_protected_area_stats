...
def calc_unq_val_pxl_areas(vals_img, pix_area_img, uid_img, unq_val_area_lut):
    ...
    if numpy.sum(msk) > 0:
        ...
        unq_val_area_lut['carbon'] = numpy.sum(vals_arr[msk])
        unq_val_area_lut['carbon_area'] = numpy.sum(vals_arr[msk] * pxl_area_arr[msk])
...
class PerformAnalysis(PBPTQProcessTool):
    ...
    def do_processing(self, **kwargs):
        ...
        lut_vals['area'] = 0.0
        lut_vals['carbon'] = 0.0
        lut_vals['carbon_area'] = 0.0
        lut_vals['hist'] = numpy.zeros((201), dtype=numpy.uint32)

        calc_unq_val_pxl_areas(self.params['carbon_img'], self.params['pxl_area'], self.params['roi_img'], lut_vals)

        lut_vals['count'] = int(lut_vals['count'])
        lut_vals['area'] = float(lut_vals['area'])
        lut_vals['carbon'] = float(lut_vals['carbon'])
        lut_vals['carbon_area'] = float(lut_vals['carbon_area'])

        rsgislib.tools.utils.write_dict_to_json(lut_vals, self.params['out_file'])

    def required_fields(self, **kwargs):
        return ["carbon_img", "pxl_area", "roi_img", "out_file"]
